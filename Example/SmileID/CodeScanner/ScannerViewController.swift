    ...
    UIAdaptivePresentationControllerDelegate
  {
        ...
        for feature in features as! [CIQRCodeFeature] {
          ...
          if qrCodeLink == "" {
            ...
            let result = ScanResult(
              string: qrCodeLink, type: .qr, image: qrcodeImg, corners: corners)
            found(result)
          }
        }
      } else {
    ...
    public func presentationControllerDidDismiss(_ presentationController: UIPresentationController)
    {
      ...
      private func setupCaptureDevice() {
        ...
        guard let videoCaptureDevice = parentView.videoCaptureDevice ?? fallbackVideoCaptureDevice
        else {
          return
        }
    ...
    func updateViewController(
      isTorchOn: Bool, isGalleryPresented: Bool, isManualCapture: Bool, isManualSelect: Bool
    ) {
      guard let videoCaptureDevice = parentView.videoCaptureDevice ?? fallbackVideoCaptureDevice
      else {
        return
    ...
    public func metadataOutput(
      ...
      if let metadataObject = metadataObjects.first {
        ...
        handler = { [self] image in
          let result = ScanResult(
            string: stringValue, type: readableObject.type, image: image,
