...
public final class SelfieViewModel: ObservableObject, ARKitSmileDelegate {
	...
	private let visionQueue = DispatchQueue(label: "com.smileid.selfieVision")
	private var sequenceRequestHandler = VNSequenceRequestHandler()
	private var faceTrackingRequest: VNTrackObjectRequest?
	private var faceLostFrameCount = 0
	...
	func analyzeImage(image pixelBuffer: CVPixelBuffer) {
		...
		if let trackingRequest = faceTrackingRequest {
			visionQueue.sync {
				do {
					...
					if let observation = trackingRequest.results?.first as? VNDetectedObjectObservation,
						 observation.confidence > Constants.faceTrackerConfidenceThreshold,
						...
						faceLostFrameCount += 1
						if faceLostFrameCount >= Constants.faceLostMaxFrames {
							resetTrackerAndUI()
						}
					}
				} catch {
					debug(
						"Sequence tracking error: \(error.localizedDescription)",
	...
	private func handleDetectionResults(
		...
		switch faces.count {
		case 1:
			if faceTrackingRequest == nil {
				let initial = VNDetectedObjectObservation(boundingBox: faces[0].boundingBox)
				let req = VNTrackObjectRequest(detectedObjectObservation: initial)
				req.trackingLevel = .accurate
				faceTrackingRequest = req
			}
		default:
			// multiple or zero faces – abandon tracker to force fresh detection next frame.
			faceTrackingRequest = nil
			faceLostFrameCount = 0
		}
