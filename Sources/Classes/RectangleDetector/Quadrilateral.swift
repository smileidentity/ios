...
struct Quadrilateral: Transformable {
  ...
  init(cgRect: CGRect) {
    topLeft = cgRect.topLeft
    topRight = cgRect.topRight
    bottomLeft = cgRect.bottomLeft
    bottomRight = cgRect.bottomRight
  }
  ...
  func toCartesian(withHeight height: CGFloat) -> Quadrilateral {
    let topLeft = topLeft.cartesian(withHeight: height)
    let topRight = topRight.cartesian(withHeight: height)
  ...
  private func sortPointsByYValue(_ points: [CGPoint]) -> [CGPoint] {
    points.sorted { point1, point2 -> Bool in
      point1.y < point2.y
    }
  }
...
extension CGPoint {
  ...
  func surroundingSquare(withSize size: CGFloat) -> CGRect {
    CGRect(x: x - size / 2.0, y: y - size / 2.0, width: size, height: size)
  }
  ...
  func isWithin(delta: CGFloat, ofPoint point: CGPoint) -> Bool {
    (abs(x - point.x) <= delta) && (abs(y - point.y) <= delta)
  }
